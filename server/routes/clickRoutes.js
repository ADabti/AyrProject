...
router.post('/click', authenticate, async (req, res) => {
  try {
      ...
      const user = await User.findByIdAndUpdate(req.user._id, { $inc: { count: 1 } }, { new: true });
      if (!user) {
        return res.status(404).send("User not found");
      }
...
router.get('/leaderboard', authenticate, async (req, res) => {
  try {
    const topUsers = await User.find().sort({ count: -1 }).limit(3);
    const currentUser = req.user;

    // Check if current user is in topUsers
    let currentUserInTop = topUsers.findIndex(user => user._id.equals(currentUser._id)) !== -1;
    let currentUserData = null;

    if (!currentUserInTop) {
      // Find current user's rank
      const allUsers = await User.find().sort({ count: -1 });
